...
All curves show MEAN across trajectories (no std shading).
...
for col, (sname, sfn) in enumerate(spec_fns):
    ...
    for t, cgt, cpred in [(1, 'royalblue', 'deepskyblue'), (20, 'darkred', 'salmon')]:
        gt_mean = spec_data[(t, 'gt', sname)].mean(axis=0)
        ar_mean = spec_data[(t, 'pred_ar', sname)].mean(axis=0)
        ...
        E_ar_all = spec_data[(t, 'pred_ar', sname)][:, :n]
        E_gt_all = spec_data[(t, 'gt', sname)][:, :n]
        rel_err = np.abs(E_ar_all - E_gt_all) / (E_gt_all + em.EPS)
...
fig.suptitle(f'KE spectra: GT vs SPUS (autoregressive) — mean over {N} trajectories', fontsize=13)
fig.tight_layout()
...
# ================================================================
# 2. KE spectra evolution at t=1, 5, 10, 15, 20 (mean only)
# ================================================================
...
for row, t in enumerate(times_evo):
    for col, (sname, _) in enumerate(spec_fns):
        ...
        for label, tag, color, ls in [('GT', 'gt', 'black', '-'),
                                       ('AR', 'pred_ar', 'red', '--'),
                                       ('OS', 'pred_os', 'blue', ':')]:
            mean = evo_data[(t, tag, sname)].mean(axis=0)
...
fig.suptitle(f'KE spectra evolution: mean over {N} trajectories', fontsize=13)
fig.tight_layout()
...
# ================================================================
# 3. Conservation rewards over time — mean only
# ================================================================
...
for ax, (title, ar_data, os_data) in zip(axes, [
    ...
    ar_m = ar_data.mean(axis=0)
    os_m = os_data.mean(axis=0)
...
axes[0].set_ylabel('reward  (-|delta|/|input|)')
fig.suptitle(f'Conservation rewards: mean over {N} trajectories', fontsize=13)
fig.tight_layout()
...
# ================================================================
# 4. Per-field L2 error growth over time — mean only
# ================================================================
...
fig.suptitle(f'Per-field error growth: mean over {N} trajectories', fontsize=13)
fig.tight_layout()
...
# ================================================================
# 6. Density power spectra at t=20 — mean only (GT, AR, OS)
# ================================================================
...
for col, (sname, _) in enumerate(spec_fns):
    ...
    for label, tag, color, ls in [('GT', 'gt', 'black', '-'),
                                   ('AR', 'pred_ar', 'red', '--'),
                                   ('OS', 'pred_os', 'blue', ':')]:
        mean = rho_spec[(tag, sname)].mean(axis=0)
...
fig.suptitle(f'Density power spectra at t={t_rho}: mean over {N} trajectories', fontsize=13)
fig.tight_layout()
